...
const EditMujairC= ({route}) => {
  const {user} = route.params;
  const {ruangan} = user;
  const navigation = useNavigation<StackNavigationProp<ParamListBase>>();
  const titleRuangan = 'Mujair C';
...
const handleBackPress = () => {
  ...
  const fetchJumlahBed = async () => {
      try {
        ...
        // Akses langsung key "jumlah bed Mujair B"
        if (result.status === 'success' && result['jumlah bed Mujair C'] !== undefined) {
          setJumlahTempatTidur(result['jumlah bed Mujair C'].toString());
        } else {
