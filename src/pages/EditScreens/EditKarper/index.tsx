...
const EditKarper = ({route}) => {
  const {user} = route.params;
  const {ruangan} = user;
  const navigation = useNavigation<StackNavigationProp<ParamListBase>>();
  const titleRuangan = 'Karper';
  ...
  const fetchJumlahBed = async () => {
      try {
        ...
        if (result.status === 'success' && result['jumlah bed Karper'] !== undefined) {
          setJumlahTempatTidur(result['jumlah bed Karper'].toString());
        } else {
