...
import { StyleSheet, Pressable, View, Text, Dimensions, ScrollView, TouchableOpacity } from "react-native";
...
const BottomSheetRuangan = ({ isVisible, onClose }) => {
  ...
  const cardButtons = [
    "Mujair A", "Mujair B", "Mujair C", "Karper", "Neonati",
    "Nike", "Bomboya", "Payangka", "ICU"
  ];
  ...
  return (
    <View style={styles.BottomSheet}>
      ...
      <View style={[styles.BottomSheetMainContainer, { width: screenWidth, height: isLandscape ? screenHeight * 0.9 : screenHeight * 0.5 }]}>
        <View style={styles.closerBar} />
        <Text style={styles.contentText}>Filter Berdasarkan Ruangan</Text>

        <ScrollView
          contentContainerStyle={styles.cardButtonContainer}
          showsVerticalScrollIndicator={!isLandscape}
        >
          {cardButtons.map((label, index) => (
            <TouchableOpacity
              key={index}
              style={[
                styles.cardButton,
              ...
              onPress={() => navigation.navigate(`Stats${label.replace(" ", "")}`)}
            >
...
export default BottomSheetRuangan;
